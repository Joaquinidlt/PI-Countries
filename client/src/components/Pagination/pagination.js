...
import { getAllActivity, allCountries, activityFilter,activitiesAll } from '../../reducer/actions';
...
export const Pagination = () => {
    ...
    const activities = useSelector(s => s.activitiesAll)
    ...
    const AreaAsc = (a, b) => { return b.area - a.area}
    ...
    useEffect(() => {
        switch (order) {
            ...
            case 'AreaAsc': setPage(0); return setFilter([...countriesFiltered].sort(AreaAsc))
        ...
        <div>
        {
            ...
            <div>
                ...
                <div className="content-select">
                    <div>
                        {page > 1 ? <button className='btn-first' onClick={firstPage}>{<MdFirstPage/>}</button> : ""}
                        {page > 1 ? <button className='btn-prev' onClick={prevPage}>{<MdKeyboardArrowLeft/>}</button> : ""}
                        {page < 240 ? <button className='btn-next' onClick={nextPage}>{<MdKeyboardArrowRight/>}</button> : ""}
                        {page < 240 ? <button className='btn-last' onClick={lastPage}>{<MdLastPage/>}</button> : ""}
                    </div>
                    <br/>
                    <select onChange={handleOrder}>
                        ...
                        <option value='AreaAsc'> Area Asc </option>
                        <option value='AreaDsc'> Area Dsc </option>
                    </select>
